...
%             TxmBxn = TxmBxn+TxmBxnsp;
        ...
        function obj = changeScaleFactor(obj,sf)
            %changes the scale factor and resets the current
            %this is going to be needed later, because my current is off
            obj.scaleFactor = sf;
            obj.JJ = obj.JJ/sf;
        end
...
%             ey = zeros(BynSize,1);
            ...
            ex_theta = pi*del * (cos(thetaLoc)*cos(phiLoc))...
                              * sinc(del*sin(thetaLoc)*cos(phiLoc))...
                              * exp(1j*kk*(obj.Bxn_xx*sin(thetaLoc)*cos(phiLoc)...
                                       +obj.Bxn_yy*sin(thetaLoc)*sin(phiLoc)));
            ey_theta =pi*del * (cos(thetaLoc)*sin(phiLoc))...
                          * sinc(del*sin(thetaLoc)*sin(phiLoc))...
                          * exp(1j*kk*(obj.Byn_xx*sin(thetaLoc)*cos(phiLoc)...
                                      +obj.Byn_yy*sin(thetaLoc)*sin(phiLoc)));
                                 
            %now phi
            ex_phi = pi*del * (-sin(phiLoc))...
                         * sinc(del*sin(thetaLoc)*cos(phiLoc))...
                         * exp(1j*kk*(obj.Bxn_xx*sin(thetaLoc)*cos(phiLoc)...
                                 +obj.Bxn_yy*sin(thetaLoc)*sin(phiLoc)));
            ey_phi =pi*del * (cos(phiLoc))...
                        * sinc(del*sin(thetaLoc)*sin(phiLoc))...
                        * exp(1j*kk*(obj.Byn_xx*sin(thetaLoc)*cos(phiLoc)...
                                 +obj.Byn_yy*sin(thetaLoc)*sin(phiLoc)));
            %---- get ZZ --------
...
%    
        ...
        function [E_theta, E_phi] = getEincComponents(obj)
            ...
            %kk/(2*pi) = 1 -> what happened to kk, /2, /pi
            %ie) yes sarah its del/2 but /2 cancelled out
            ex = del * (cos(thetaLoc)*cos(phiLoc))...
                         * sinc(del*sin(thetaLoc)*cos(phiLoc))...
                         * exp(1j*kk*(obj.Bxn_xx*sin(thetaLoc)*cos(phiLoc)...
                                    +obj.Bxn_yy*sin(thetaLoc)*sin(phiLoc)));
            ey =del * (cos(thetaLoc)*sin(phiLoc))...
                         * sinc(del*sin(thetaLoc)*sin(phiLoc))...
                         * exp(1j*kk*(obj.Byn_xx*sin(thetaLoc)*cos(phiLoc)...
                                    +obj.Byn_yy*sin(thetaLoc)*sin(phiLoc)));
            E_theta = [ex ey].';
...
%         
        ...
        function [theta,rcs_tt,rcs_tp,rcs_pt,rcs_pp] = plotRCS(obj,phi)
