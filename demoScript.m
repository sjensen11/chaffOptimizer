...
pixelNum = NumCells_odd/pixelSize 
% lt = randi([0 1],4);
% nullPixelLT = tril(lt)
...
%% plot flat rcs for comparision
[rcsNullTT,rcsNullPP,rcsFullTT,rcsFullPP] = chfNulled.plotMonoFlat(0); %dB off
...
%% to feko mark
chfNulled = chf_odd;
FILENAME ='JUSTPLATE.pre'
maxCells = chfNulled.plateNull.NumCells;

nullPos = chfNulled.nullPos;

bitpattern = ones(maxCells);
bitpattern(nullPos) = 0;

thetainc =linspace(0,90,5);
phiinc = 0;

W = chfNulled.plateLength/chfNulled.plateFull.NumCells;
fo = chfNulled.freq;

write_chaff_FEKO(FILENAME,bitpattern,W,fo,thetainc,phiinc)